...
public class UserServiceImpl implements UserService {
    ...
    public List<ArticleDTO> selectArticleByAuthor() {
        try {
            Authentication authentication=SecurityContextHolder.getContext().getAuthentication();
            User loginUser= (User) authentication.getPrincipal();
            Long userId = loginUser.getId();
            List<ArticleDTO> articles=userMapper.selectArticleByAuthor(userId);
    ...
    public List<ArticleDTO> selectMyLike() {
        try {
            Authentication authentication=SecurityContextHolder.getContext().getAuthentication();
            User loginUser= (User) authentication.getPrincipal();
            Long userId = loginUser.getId();
            List<ArticleDTO> articles=userMapper.selectMyLike(userId);
            for(ArticleDTO article:articles){
                article.setUsername(loginUser.getUsername());
            }
            return articles;
